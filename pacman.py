...
# DB-backed high score for launcher user
high_score = 0
if LAUNCHER_USER:
    try:
        import db
        high_score = db.get_user_best_for_game(LAUNCHER_USER, 'Pacman') or 0
    except Exception:
        high_score = 0
...
# Load sound for power-pellet (safe fallbacks)
try:
    pygame.mixer.init()
    try:
        sfx_eat = pygame.mixer.Sound('sound/sfx_eat.wav')
    except Exception:
        try:
            sfx_eat = pygame.mixer.Sound('sound/sfx_point.wav')
        except Exception:
            sfx_eat = None
except Exception:
    sfx_eat = None
...
def draw_misc():
    ...
    # draw DB-backed high score
    try:
        hs = int(high_score)
    except Exception:
        hs = 0
    hs_text = font.render(f'High Score: {hs}', True, 'white')
    screen.blit(hs_text, (200, 920))
    if powerup:
...
while run:
    ...
    if powerup:
        ...
        # increase Pac-Man speed a bit while powerup is active
        player_speed = 3
    else:
        ghost_speeds = [2, 2, 2, 2]
        player_speed = 2
        ...
        if event.type == pygame.QUIT:
            ...
            try:
                if AUTO_RECORD and LAUNCHER_USER and not _RECORDED:
                    import db
                    db.record_score('Pacman', LAUNCHER_USER, int(score))
                    # refresh high score after recording
                    try:
                        high_score = db.get_user_best_for_game(LAUNCHER_USER, 'Pacman') or high_score
                    except Exception:
                        pass
                    _RECORDED = True
            except Exception:
